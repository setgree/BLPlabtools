#' tidy_lm_results
#'
#' Run multiple linear model specifications and return results in a tidy data frame
...
tidy_lm_results <- function(data, dv, terms, treatment = NULL, style = "default", clusters = NULL, robust_se = FALSE, alpha = .05, multiple_testing = NULL, print_summary = FALSE){
  ...
  if (is.null(treatment) == FALSE){
    ...
    for(i in 1:length(treatment)) {
      ...
      test <- try(inherits(data[[treatment[i]]], "factor"), silent = T)
      if (!inherits(test, "try-error")){
