...
`map_robust` is a wrapper around a pre-existing function from the `metafor` package called `robust`, which is itself a wrapper around the function `rma`. See the `man` pages for those functions for full details.
...
Our meta-analyses cluster at the level of study. You could also cluster at the level of a paper, or of a team of authors, or at no level at all. If you run our function on a dataset where each `unique_study_id` corresponds to only one row in the dataset, the results are nearly identical to not clustering---the point estimates are the same, and the standard errors and p-values differ only in the third or fourth decimal place.^[The small differences arise because `robust()` uses a sandwich variance estimator with a small-sample correction, whereas `rma()` uses model-based standard errors, and because `robust()` uses a *t*-distribution for inference while `rma()` uses a *z*-distribution.] We'll demonstrate that with `BLPlabtools::contact_data`:
...
# nearly the same as if we drop the clustering information
...
`dplyr::bind_rows()` combines multiple data frames (in this case, a list of lists) into a single data frame.
