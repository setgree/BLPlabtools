...
1. `tidy_lm_results` for running multiple regression specifications efficiently
...
For experimental analysis with `tidy_lm_results` and `robust_se`, your data should be in "long" format with one row per observation (student, participant, unit). Here's what you need:
...
The `tidy_lm_results` function is designed to work with **tidy data** where:
...
**A note on clustering**: If your experimental units are nested (students within schools, voters within precincts), you should account for this by specifying the `clusters` parameter in `tidy_lm_results` or using `robust_se` directly. This adjusts standard errors to account for within-cluster correlation and prevents false precision in your estimates.

## 1 Running multiple regressions with `tidy_lm_results`

When analyzing experimental data, you often need to run many regression specifications: different dependent variables, different sets of controls, different subsamples. The `tidy_lm_results` function streamlines this process.
...
### 2.2 Incorporating clusters into `tidy_lm_results`

For multiple specifications with clustering, use `tidy_lm_results` directly:

```{r tidy_lm_clusters}
# Run multiple specifications with clustering
clustered_models <- tidy_lm_results(
  data = experiment_data,
...
The `star_ready` function prepares `tidy_lm_results` output for use with the `stargazer` package:
...
- `nothing()`: Does nothing—useful as a placeholder at the end of pipe chains
...
### 3.3 Typical workflow for publication-ready tables
...
- Use `tidy_lm_results` when you need to run many specifications
...
**For creating tables:**
- `star_ready` bridges `tidy_lm_results` output to `stargazer`
