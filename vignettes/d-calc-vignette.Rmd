...
In plain language: an SMD is the difference in average outcomes between the treatment and control groups (Average Treatment Effect) divided by the standard deviation (SD) of the outcome. (**Important: standard deviation is not the same thing as standard error! Dividing the effect size by standard error will give you effect sizes that are much too big, This sometimes happens [in published research](https://pmc.ncbi.nlm.nih.gov/articles/PMC9877053/).**)
...
In one instance — the difference in proportions estimator — we depart from those equations, which we'll explain in [the log odds and difference in proportions section](#log-odds-diff-proportions).
...
When the $SD$ of the control group isn't available but the $SD$ of the entire population is, we use the $SD$ of the entire population. When neither is available, or can't be figured out from a paper's results, we might have a problem. We discuss these issues more in [the hard cases section](#hard-cases).

### What your data should look like

Before diving into effect size calculations, it helps to know what columns you'll need in your meta-analytic database. Here's a view of the `contact_data` structure:
...
**Essential columns you'll need to extract from each paper:**
...
As you read papers and extract data, you'll fill in the "essential" columns. Then you can use `d_calc` and `var_d_calc` (as shown in [the automation section](#automating-d-calc)) to automatically generate the effect sizes and variances for your entire dataset.

The rest of this vignette walks through examples of calculating effect sizes one study at a time. In practice, you'll eventually automate this process for your entire dataset (see [automating d_calc](#automating-d-calc)).
...
### The easy cases: difference in means and regression coefficients
...
```{r ditullio_results}
ditullio_results <- d_calc(stat_type = "d_i_m", stat = 5.708 - 3.0789, sample_sd = 1.0381)
...
In a bivariate regression testing the relationship between treatment status and outcome, the $\beta$ coefficient is equivalent to the difference in means between the groups. Calculating the $SD$ from a regression table takes some guesswork (see [the hard cases section](#hard-cases)), but some studies report both $\beta$ and the $SD$ for the outcome.
...
Bois_var_black_roommates <- var_d_calc(d = Bois_d_black_roommates,
                                       n_t = black_roommate_treatment_n,
                                       n_c = boisjoly_n_c)
Bois_se_black_roommates <- sqrt(Bois_var_black_roommates)

# other minority roommates
Bois_d_other_roommates <- d_calc(stat_type = "reg_coef", stat = .032, sample_sd = 1.074)
Bois_var_other_roommates <- var_d_calc(d = Bois_d_other_roommates,
                                       n_t = other_minority_treatment_n,
                                       n_c = boisjoly_n_c)
Bois_se_other_roommates <- sqrt(Bois_var_other_roommates)
```
...
In these cases, we subtract the baseline values from the posttest values for both treatment and control; subtract the change in the control group from the change in the treatment group; and divide the resulting difference by the $SD$ of the control group at baseline.

This procedure creates more precise estimates because it controls for baseline differences between the two groups.
...
```{r clunies_effects}
clunies_a <- d_calc(stat_type = "d_i_d", stat = (111.1-92.7)-(102.9-99.4), sample_sd = 28.28)

clunies_a_var <- var_d_calc(d = clunies_a, n_t = 15, n_c = 15)

clunies_a_se <- sqrt(clunies_a_var)
```

Notice that we record the DiD calculation in the code itself for reproducibility (and also to make it easier to spot mistakes). Alternatively, you might record it as a formula in your data spreadsheet. It's good to have a record of it somewhere.

#### A few DiD-related judgment calls

-   When possible, we prefer differences rather over differences in means for calculating d or ∆. Including baseline/pretest information should increase the precision of the resulting estimate by incorporating information about changes over time that (in theory) affect both groups.

-   DiD sometimes requires interpreting data from multiple tables. Papers occasionally present the treatment group's change over time in one table and the control group's change over time in another. This makes the meta-analyst's job a little harder, but the logic is the same.

-   When multiple SDs are available, we typically take the SD of the control group **at pretest/baseline** as our "standardizing" value. This is not essential, but it's a helpful convention.
...
#### log odds ratios, odds ratios, and a novel difference in proportions estimator: {#log-odds-diff-proportions}
...
*n.b.* You can also use the R package `metansue` to address this general problem. (We used the `unspecified_null = 0.01` convention in our meta-analyses where it came up.)

## Hard cases: non-standard conversions of results into SMDs {#hard-cases}
...
## Incorporating `d_calc` and `var_d_calc` into your analysis scripts {#automating-d-calc}
